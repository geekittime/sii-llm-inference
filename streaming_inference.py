...
# -*- coding: utf-8 -*-
"""
streaming_inference.py - 改进版
流式输出推理演示
"""

import time
import torch
from optimized_inference_v2 import load_model, get_eos_ids
...
def streaming_generate(model, tokenizer, prompt: str, max_new_tokens: int = 256):
    ...
    inpt_mask = enc.get("attention_mask", torch.ones_like(input_ids))
...
def main():
    ...
    tokenizer, model, kv_cache = load_model(args.model_path)
    ...
    print("\n" + "=" * 60)
    print(f"[完成] 峰值显存: {torch.cuda.max_memory_allocated(DEVICE)/1e9:.2f} GB")

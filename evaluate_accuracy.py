...
用法:
  python evaluate_accuracy.py --model_path /path/to/model --batch_size 16
"""
...
from optimized_inference import load_model, infer_all, BATCH_SIZE
...
def run_accuracy_eval(tokenizer, model, eval_data: list, batch_size: int) -> dict:
    ...
    results = infer_all(
        ...
        max_new_tokens=32,           # 只需要输出一个字母，不用长生成
        show_progress=True,
    )
...
def print_result(r: dict, baseline: float = None):
    ...
    if baseline is not None:
        ...
        ok   = "✓ 达标" if drop <= ACCURACY_DROP_LIMIT else "✗ 超标"
...
def main():
    parser = argparse.ArgumentParser(description="C-Eval 精度评测（批量版）")
